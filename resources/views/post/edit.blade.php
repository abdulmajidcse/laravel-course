<x-layout>
    ...
    <div class="card">
        <div class="card-header">
            <h2>Edit Post <a href="{{ route('posts.index') }}"
            ...
            <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('category') is-invalid @enderror" name="category" id="category" required>
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" @if($category->id == old('category', $post->category_id)) selected @endif>{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @error('category')
                ...
                <div class="mb-3">
                    ...
                    @if (Storage::exists($post->image))
                        <div class="p-2">
                            <img class="img" style="width: 300px;" src="{{ Storage::url($post->image) }}" alt="Image">
                        </div>
                    @endif
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea rows="3" class="form-control @error('body') is-invalid @enderror" name="body" id="body" required>{{ old('body', $post->body) }}</textarea>
                    @error('body')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
